...
export const hookxuatKhoKhoa: CollectionBeforeChangeHook= async ({ data, req, operation,originalDoc}) => {
  ...
  let updatedInventory = [...department.departmentInventory];
...
export const autoCalculateTotalPrice: CollectionBeforeChangeHook = async ({ data }) => {
  ...
  const calculateTotalForItems = (items: any[]) => {
    return items?.map((item) => {
      ...
      let unitPrice = parseFloat(item.unitprice.replace(/\./g, "")); // Loại bỏ dấu chấm (nếu có)
  ...
  if (data.danhsachsanpham) {
    ...
    if (data.danhsachsanpham.maymocthietbi) {
      data.danhsachsanpham.maymocthietbi = calculateTotalForItems(data.danhsachsanpham.maymocthietbi);
    }
  }
...
export const hookcheckvalue: CollectionBeforeChangeHook = async ({ data, req, operation }) => {
  ...
  if (data.danhsachsanpham) {
    ...
    // Kiểm tra thuốc
    if (thuoc && thuoc.some((item, index) => {
    ...
    if (vattutieuhao && vattutieuhao.some((item, index) => {
      let missingFields: string[] = []; // Chỉ định kiểu mảng là string[]
    ...
    if (maymocthietbi && maymocthietbi.some((item, index) => {
      let missingFields: string[] = []; // Chỉ định kiểu mảng là string[]
...
export const checkInventoryBeforeUsage = async ({ data, req }: { data: any, req: PayloadRequest }) => {
  ...
  department.departmentInventory.forEach((item) => {
    const medicationsId = typeof item.item === 'object' && item.item  !== null ? item.item.id : item.item
    const supplyid = typeof item.items === 'object' && item.items  !== null ? item.items.id : item.items
  ...
  itemsToCheck.forEach(({ category, itemId, requiredQuantity }) => {
      ...
      console.log(`🔍 Kiểm tra sản phẩm: ${itemId}, cần ${requiredQuantity}, tồn kho ${availableQuantity}`);

      if (requiredQuantity > availableQuantity) {
          console.log(`❌ Không đủ số lượng: ${itemId}`);
          errors.push(`Không đủ số lượng cho sản phẩm: ${itemId} (cần ${requiredQuantity}, tồn kho ${availableQuantity})`);
      } else {
